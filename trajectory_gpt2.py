...
class GPT2Model(GPT2PreTrainedModel):
    def __init__(self, config):
        ...
        self.h = nn.ModuleList([Block(config.n_embd, config, scale=True) for _ in range(config.n_layer)])
...
class Autodis(nn.Module):
    def __init__(self, config, bucket_number):
        ...
        self.ret_emb_score = nn.Sequential(nn.Linear(1, bucket_number), nn.LeakyReLU())
        self.res = nn.Linear(bucket_number, bucket_number)
        self.temp = nn.Sequential(
            nn.Linear(1, bucket_number),
            nn.LeakyReLU(),
            nn.Linear(bucket_number, bucket_number),
            nn.Sigmoid()
            )
...
class CNN(nn.Module):
    ...
    def forward(self, x):
        x = x.unsqueeze(1)
        if self.is_train:
            output_data = self.conv(x)
            x1 = self.conv1(output_data)
            # x1 = self.conv1(x)
            x1 = self.relu1(x1)
            x1 = self.conv2(x1)
            x1 = self.relu2(x1)
...
class VectorPatchEmb(nn.Module):
    def __init__(self, config):
        ...
        self.action_emb = nn.Linear(6300, iD)
    ...
    def forward(self, states, actions, rewards=None):
        ...
        B, T, C = states.size()
        local_state_tokens = (
                    self.patch_embedding(states.unsqueeze(-1)).reshape(B * T, C, -1) + self.spatial_emb).reshape(B, T,-1,16)
        ...
        local_action_tokens = self.action_emb(actions.reshape(B, T, -1)).unsqueeze(2)  # B T 1 iD
        #         print (local_action_tokens.size(), local_state_tokens.size())
        if 'rwd' in self.config.model_type:
            local_reward_tokens = self.reward_emb(rewards.reshape(-1, 1)).reshape(B, T, 84).unsqueeze(2)
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config, N_head=1, D=64, T=30):
        super().__init__()
        assert D % N_head == 0
        self.config = config
...
class SABlock(nn.Module):
    def __init__(self, config):
        ...
        patch_count = 84
        maxT = 1000
        self.local_block = _SABlock(config, 4, 16)
        self.global_block = _SABlock(config, 1, 64)
        if 'fusion' in config.model_type:
            self.register_buffer("mask", torch.tril(torch.ones(maxT * 2, maxT * 2))
                                 .view(1, 1, maxT * 2, maxT * 2))
        else:
            self.register_buffer("mask", torch.tril(torch.ones(maxT * 2, maxT * 2))
                                 .view(1, 1, maxT * 2, maxT * 2))
            for i in range(0, maxT * 2, 2):
                self.mask[0, 0, i, i - 1] = 1.
        self.local_norm = nn.LayerNorm(16)
        ...
        self.local_global_proj = nn.Sequential(
            nn.Linear(total_patch_count * 16, 64),  # +1 for action token +1 for reward token
            nn.LayerNorm(64)
        )
    ...
    def forward(self, local_tokens, global_tokens, temporal_emb=None):
        ...
        if 'fusion' in self.config.model_type or 'xconv' in self.config.model_type:
            ...
            if temporal_emb is not None:
                lt_tmp += temporal_emb
            global_tokens = torch.stack((lt_tmp, global_tokens), dim=2).view(B, -1, 64)
            global_tokens, global_att = self.global_block(global_tokens, self.mask)
...
class GPTConfig:
    ...
    def __init__(self, vocab_size, block_size,n_embd, n_layer,max_timestep,**kwargs):
        ...
        for k,v in kwargs.items():
            setattr(self, k, v)
class CausalSelfAttention1(nn.Module):
    """
