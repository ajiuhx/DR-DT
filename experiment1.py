...
from decision_transformer import DecisionTransformer,FeatureExtractor
...
f_true = open('/home/hx/ORB_SLAM3/truth_deal/true_corridor2.txt', 'r')
...
def get_observation(done=False):
    ...
    n = int(fields[0])
    with open('/home/hx/ORB_SLAM3/Examples/Monocular/TUM_TimeStamps/dataset-corridor2_512.txt', 'r') as file:
        # with open('/home/hx/ORB_SLAM3/Examples/Monocular/TUM_TimeStamps/dataset-room4_512.txt', 'r') as file:
        ...
        # 换数据集要修改
    im = Image.open('/home/hx/ORB_SLAM3/data/dataset-corridor2_512_16/mav0/cam0/data/' + line_content + '.png')
    # 换数据集要修改
...
def getError(fields, done):
    ...
    while count < len(true_traj):
        ...
        #corridor2:1520616232707170000
        if fields_true[0] == timeStep_temp:  # 如果估计轨迹的时间戳等于真实轨迹的时间戳
            if done and (timeStep_temp != "1520616232707170000"):
    ...
    if len(ground) > 2:
        ...
        rmse = 0.50001
    # 换数据集要修改
    if len(ground) == 1375:  # mh01 = 3637  MH02=2995   MH03=2624   mh04=1965  mh05=1509
...
def run_maze(variant):
    ...
    store = Store(state_dim=state_dim,
        ...
        mem_size=20000)
    #使用mh01训练
    for episode in range(6727):#MH01 = 3680  MH02=3040  MH03=2700  mh04=2033  mh05=2273
        ...
        while True:
            ...
            # states = observation.reshape(1, -1, state_dim).float()
